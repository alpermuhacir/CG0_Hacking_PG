// CGO_Hacking_Proc.cpp : Bu dosya 'main' işlevi içeriyor. Program yürütme orada başlayıp biter.
//
...
struct PModule { DWORD dwBase; DWORD dwSize; };
...
public:
	...
	bool Attach(char* pName, DWORD rights)
	{
		...
		do if (!strcmp(entry.szExeFile, pName))
		{
	...
	template <class T> T Read(DWORD addr)
	{
		T _read;
	...
	template <class T> void Write(DWORD addr, T val)
	{
	...
	{
		BYTE* data = new BYTE[size]; unsigned long bytesRead;
	...
	{
		char* sig = new char[count + 1]; va_list ap; va_start(ap, count);
...
private:
	...
	/* get offset to client.dll */
public: static void get_offset(process* m)
{
	mem = m;
	...
	update_glow();
		...
		/*constantly scanning & updating our offsets static*/
		DWORD WINAPI scan_offsets(LPVOID PARAM)
		{
			Entity players[64];
			while (true)
			{
				...
				for (int i = 1; i < 64; i++)
				{
					...
					if (entBase == NULL) continue;
					update_entity_data(&players[cp], entBase); cp++;
				}
				int cf = 0, ce = 0;
				for (int i = 0; i < cp; i++)
				{
...
int main()
{
	...
	std::cout << "F1 to switch ESP!" << std::endl;
	while (TRUE)
	{
